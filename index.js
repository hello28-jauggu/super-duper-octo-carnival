(async () => {
  ...
  const tanks = {
    ...
    physician: { path: [[2, 3], [3, 3]], build: "12/12/0/0/0/0/12/0/3/3" },
  ...
  const arras = (function () {
    ...
    const getStatus = function (f, s) {
      ...
      if (statusData && now - lastStatus < 20000) {
        return {
          then: function () {
            return {
              then: function (f) {
    ...
    const run = function (x, config, oa) {
      ...
      let _lastPathDir = null;
      function pathfind(x, y) {
        const dx = x - position[0];
        const dy = y - position[1];
        const dist2 = dx * dx + dy * dy;
        // Stop if already close enough (avoids jitter/key spam)
        if (dist2 < 0.36) {  // OPT: stop earlier, less key spam
          if (_lastPathDir !== null) {
            stopMoving();
            _lastPathDir = null;
          }
          return;
        ...
        let h = (Math.round(angle * 4 / Math.PI) % 8 + 8) % 8;
        if (h === _lastPathDir) return; // same direction already held
        _lastPathDir = h;
        const dirs = [
          {D:1}, {S:1,D:1}, {S:1}, {S:1,A:1},
          {A:1}, {W:1,A:1}, {W:1}, {W:1,D:1}
        ][h];
        for (const k of "WASD") {
          const code = "Key" + k;
          trigger[dirs[k] ? "keydown" : "keyup"](code);
        }
      }
        ...
        if (a) {
          ...
          if (lastHash !== global.location.hash) {
            ...
            if (config.hash !== newHash) {
              config.hash = newHash;
              sendParent({ type: 'hash_update', id: config.id, hash: newHash });
            }
          }
          let at = timeouts[i]
          if (at) {
          ...
          if (inGame && config.type === 'follow') {
            ...
            // Skip all aim work when noAim (saves CPU at 200+ bots)
            if (!target.noAim && (valid || target.noMove)) {
              let angle;
              if (target.shift) {
                angle = Math.atan2(target.mouseY || 0, target.mouseX || 0);
              } else {
                angle = Math.atan2(
                  (aimTarget.y || 0) - position[1],
                  (aimTarget.x || 0) - position[0]
                );
              }
              const nx = (innerWidth / 2) + Math.cos(angle) * 200;
              const ny = (innerHeight / 2) + Math.sin(angle) * 200;
              if (Math.abs(nx - controller.x) > 8 || Math.abs(ny - controller.y) > 8) {
                controller.x = nx;
                controller.y = ny;
                trigger.mousemove(nx, ny);
              }
            }

            {
              const wantL = !!(target.mouseDown && !target.feed);
              const wantR = !!(target.rMouseDown && !target.feed);
              if (wantL !== !!target._lastL) {
                controller[wantL ? "mouseDown" : "mouseUp"]();
                target._lastL = wantL;
              }
              if (wantR !== !!target._lastR) {
                controller[wantR ? "mouseDown" : "mouseUp"](2);
                target._lastR = wantR;
              }
            }
            ...
            let interval = setInterval(function () {
              ...
              if (config.autoRespawn) {
                ...
                const interv = setInterval(() => {
                  ...
                  tries++;
                  if (!died2 || tries > 15) clearInterval(interv);
                }, 2500);
              }
              block = false
              ignore = false
